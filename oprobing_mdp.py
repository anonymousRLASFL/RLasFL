...
class OProbing():
    ...
    def do_reset_time_and_state(self):
        ...
        self.t = 0
        self.model.reset()
        self.current_state = np.argmax(
            np.random.multinomial(1, self.model.get_start())
        )

    def do_initialize(self):

        self.do_reset_time_and_state()
        self.possible_actions = self.model.get_state_possible_actions()
        self.possible_actions_set = {
            k: set(self.possible_actions[k])
            for k in self.possible_actions.keys()
        }
        ...
        self.current_transition_mat = np.random.rand(
            self.model.get_num_actions(), self.model.get_num_states(),
            self.model.get_num_states()
        )
        self.current_policy = np.random.rand(
            self.model.get_num_actions(), self.model.get_num_states()
        )
        self.current_occupancy = np.zeros((
            self.model.get_num_actions(), self.model.get_num_states()
        ))

        for s in range(self.model.get_num_states()):
            for a in range(self.model.get_num_actions()):
                if a not in self.possible_actions_set[s]:
                    self.current_transition_mat[a, s, :] = 0.0
                    self.current_policy[a, s] = 0.0

        temp = self.current_transition_mat.sum(axis=2, keepdims=True)
        temp[temp == 0] = 1.0
        self.current_transition_mat = self.current_transition_mat / temp

        temp = self.current_policy.sum(axis=0, keepdims=True)
        temp[temp == 0] = 1.0
        self.current_policy = self.current_policy / temp

        self.freq_transition_mat = np.zeros((
            self.model.get_num_actions(), self.model.get_num_states(),
            self.model.get_num_states()
        ), dtype=np.float)
        self.freq_policy_mat = np.zeros((
            self.model.get_num_actions(), self.model.get_num_states()
        ), dtype=np.float)
        self.freq_state_mat = np.zeros((
            self.model.get_num_states(),
        ), dtype=np.float)

    def do_init_sigma(self):

        self.sigma = np.array([
            len(self.possible_actions[s])
            for s in range(self.model.get_num_states())
        ])

    def get_action_from_diff_policy(self):

        temp = self.freq_policy_mat[:, self.current_state].sum()
        if temp == 0:
            temp = 1.0
        diff = self.freq_policy_mat[:, self.current_state] / \
            temp - self.current_policy[:, self.current_state]
        diff_possible = diff[self.possible_actions[self.current_state]]
        i = np.argmin(diff_possible)
        a = self.possible_actions[self.current_state][i]

        return a

    def do_probe(self):

        if self.sigma[self.current_state] > 0:
            a = self.possible_actions[self.current_state][
                self.sigma[self.current_state] - 1
            ]
        else:
            a = self.get_action_from_diff_policy()
    ...
    def do_estimation(self):
        ...
        #i = 0
        #iMax = 10 * self.t_max * self.model.get_num_states() * self.model.get_num_actions()

        numReset = 0
        while self.sigma.max() > 0:

            a = self.do_probe()
            nxt_state = self.model.exec_one_step(self.current_state, a)

            self.freq_transition_mat[a, self.current_state, nxt_state] += 1
            self.freq_policy_mat[a, self.current_state] += 1
            self.freq_state_mat[self.current_state] += 1

            self.sigma[self.current_state] = max(
                0, self.sigma[self.current_state] - 1
            )
            ...
            if self.t > self.t_max:
                self.do_reset_time_and_state()
                numReset += 1
                if numReset > len(self.sigma):
                    break

            #print('================= sigma est: ', self.sigma)
        # print('exit do_estimation')
        return self.current_state

    def do_solve_cop(self, freq):
        ...
        v_belief = np.zeros(self.model.get_num_states() + 1, dtype=np.float)
        v_belief[self.model.get_num_states()] = 1.0
        v_belief[0:self.model.get_num_states()] = (
            1.0 - self.model.get_discount()) * self.model.get_start()
        ...
        m_dynamics[0:self.model.get_num_states(), :, :] = np.eye(
            ...
            1,
            self.model.get_num_states()
        ) - self.model.get_discount() * np.transpose(freq, (2, 0, 1))

        p = self.model.get_num_states() + 1
        q = LA.matrix_rank(m_dynamics.reshape((-1, m)))
        if q < p:
            while q < p:
                i = 0
                while i < p:
                    if i == 0:
                        ...
                        t = np.concatenate(
                            [m_dynamics[0:i], m_dynamics[i + 1:]])
                        b = np.concatenate(
                            [v_belief[0:i], v_belief[i + 1:]])
        ...
        m_dynamics = m_dynamics.reshape((-1, m))
    ...
    def do_control(self, start=None):
        ...
        rewardFun, lossFunc = None, None
        try:
            res, con, fun = self.do_solve_cop(freq_prob)  #
            val = [
                (res['x'] is not None),
                (res['x'] is not None) and (
                    (1.0 + 1e-2 - np.array(res['x']).max()) >= 0),
                (res['x'] is not None) and (
                    abs(1.0 - np.sum(np.array(res['x']))) <= 1e-2)
            ]
            # print('========================================val: ', val)
            if (res['x'] is not None) and (
                    abs(np.array(res['x']).min()) <= 1e-2
                    and (1.0 + 1e-2 - np.array(res['x']).max()) >= 0
                    and abs(1.0 - np.sum(np.array(res['x']))) <= 1e-2):

                resX = np.array(res['x'])
                sol = np.maximum(resX, 0.0).reshape((
                    self.model.get_num_actions(), self.model.get_num_states()
                ))
                self.current_occupancy = sol

                temp = sol.sum(axis=0, keepdims=True)
                temp[temp == 0] = 1.0
                self.current_policy = sol / temp

                rewardFun = fun
                lossFunc = LA.norm(con)
                norm1x = np.sum(np.absolute(resX))
                norm1y = np.sum(resX)
            else:
                rewardFun, lossFunc = None, None
                if res['x'] is not None:
                    ...
                    # print('Failure: ', res['status'])
                    pass
                # print('Failure: ', res['status'])
        except ValueError:
            pass

        temp = self.freq_policy_mat.sum(axis=0, keepdims=True)
        temp[temp == 0] = 1.0
        pol = self.freq_policy_mat / temp

        diff = pol - self.current_policy

        # print('min max current_policy: ',
        #       self.current_policy.min(), self.current_policy.max())
        # print('min max pol: ', pol.min(), pol.max())

        diff_abs = np.absolute(diff)

        temp_eps = diff_abs.max()
        temp_eps = temp_eps / 4
        temp_eps = min(temp_eps, self.eps)

        # self.temp_eps = temp_eps
        # temp_eps = max(temp_eps, self.eps_end)

        # print('max 0: ',  diff_abs.max(), ' temp_eps: ', temp_eps)
        nMax = 1000
        allMax = deque([0.0] * (nMax - 1) + [1.0], nMax)

        # self.eps:
        while (diff_abs.max() > self.eps) and (
                not (self.tolerance > 0.0 and all(
                    abs(x - allMax[0]) <= self.tolerance for x in allMax
                ))):
            ...
            allMax.append(diff_abs.max())

            if self.t > self.t_max:
                self.do_reset_time_and_state()

            if start is not None and self.max_trial_time > 0:
                elapsed = time.time() - start
                if elapsed > self.max_trial_time:
                    break

        # print('pol: ', pol)

        return self.current_state, rewardFun, lossFunc
    ...
    def solve(self, env_id, world_env, horizon, eps, tol, num_trials=1, max_trial_time=0, seed=None, output_dir='.', save_freq=100, debugFlag=False):
            ...
            for trial in range(num_trials):
                ...
                for episode in range(self.episodes):
                    ...
                    self.eps = self.eps_end + (
                        ...
                        np.exp(-self.episode / self.eps_decay)
                    )

                    self.do_init_sigma()

                    state = self.do_estimation()

                    state, reward, error = self.do_control(start)

                    if reward is None:
                        if len(allRewards) > 0 and episode > 0:
                            reward, error = allRewards[-1][2], allErrors[-1][2]
                        else:
                            reward, error = 0.0, 0.0
                    ...
                    allRewards.append([trial, episode, -reward, 0])
                    allErrors.append([trial, episode, error, 0])
                    allTimes.append([trial, episode, elapsed, 0])

                    allResults.append(
                        [trial, episode, np.absolute(self.current_occupancy).sum(), 0])
                    ...
                    if debugFlag:
                        ...
                        suf = " ] ({:.2f}%) -> loss = {:.3f}  reward = {:.3f}  eps = {:.2f}% ".format(
                            100.0 * self.episode / self.episodes,
                            error, reward, 100.0 * self.eps
                        )
                    ...
                    if episode > 0 and ((episode % save_freq == 0) or (episode == self.episodes - 1) or breakFlag):
                        ...
                        np.savetxt("%s/%s_oprobing_errors.txt" %
                                   (output_dir, env_id), allErrors)
                        np.savetxt("%s/%s_oprobing_rewards.txt" %
                                   (output_dir, env_id), allRewards)
                        np.savetxt("%s/%s_oprobing_times.txt" %
                                   (output_dir, env_id), allTimes)
                        np.savetxt("%s/%s_oprobing_occupancy.txt" %
        ...
        ax.legend(['OProbing'])
        plt.savefig("%s/%s_oprobing_errors.png" % (output_dir, env_id))
        ...
        ax.legend(['OProbing'])
        plt.savefig("%s/%s_oprobing_rewards.png" % (output_dir, env_id))
        ...
        ax.legend(['OProbing'])
        plt.savefig("%s/%s_oprobing_times.png" % (output_dir, env_id))
        ...
        ax.legend(['OProbing'])
        plt.savefig("%s/%s_oprobing_norm1.png" % (output_dir, env_id))
