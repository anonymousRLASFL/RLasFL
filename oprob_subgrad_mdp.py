...
class OProbSubgrad():
    ...
    def solve(self, env_id, world_env, horizon, learning_rate, tol, num_trials=1, max_trial_time=0, seed=None, output_dir='.', save_freq=100, debugFlag=False):
        ...
        try:
            ...
            for trial in range(num_trials):
                ...
                for episode in range(self.episodes):
                    ...
                    allRewards.append([trial, episode, reward, 2])
                    allErrors.append([trial, episode, error, 2])
                    allTimes.append([trial, episode, elapsed, 2])

                    allResults.append(
                        [trial, episode, np.absolute(self.v_occupancy).sum(), 2])
                    ...
                    if episode > 0 and ((episode % save_freq == 0) or (episode == self.episodes - 1) or breakFlag):
                        ...
                        np.savetxt("%s/%s_oprob_subgrad_errors.txt" %
                                   (output_dir, env_id), allErrors)
                        np.savetxt("%s/%s_oprob_subgrad_rewards.txt" %
                                   (output_dir, env_id), allRewards)
                        np.savetxt("%s/%s_oprob_subgrad_times.txt" %
                                   (output_dir, env_id), allTimes)
                        np.savetxt("%s/%s_oprob_subgrad_occupancy.txt" %
        ...
        ax.legend(['OProbSubgrad'])
        plt.savefig("%s/%s_oprob_subgrad_errors.png" % (output_dir, env_id))
        ...
        ax.legend(['OProbSubgrad'])
        plt.savefig("%s/%s_oprob_subgrad_rewards.png" % (output_dir, env_id))
        ...
        ax.legend(['OProbSubgrad'])
        plt.savefig("%s/%s_oprob_subgrad_times.png" % (output_dir, env_id))
        ...
        ax.legend(['OProbSubgrad'])
        plt.savefig("%s/%s_oprob_subgrad_norm1.png" % (output_dir, env_id))
...
def main():
    ...
    allRewards, allErrors = probing.solve(
        env_id, env, args.horizon, args.lr, args.tol, args.trials, args.max_trial_time,
